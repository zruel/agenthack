...
export async function appendAuditEvent(args: { actor: string; action: string; entity_type: string; entity_id: string; before_hash?: string; after_hash?: string; timestamp?: string; ip?: string }) {
  const { workbook } = await auditStore.read();
  const rows = auditStore.readRows<any>(workbook, 'Audit');
